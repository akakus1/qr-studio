...
const PLANS: { id: PlanId; price: string; popular?: boolean }[] = [
  { id: "free", price: "$0" },
  { id: "pro", price: "$9", popular: true },
    ...
    <div style={{ position: "relative" }}>
      <button
        ...
        style={{
          ...
          borderRadius: "var(--r12)", padding: "7px 12px",
          color: "var(--t2)", fontSize: ".82rem", fontWeight: 600,
      ...
      >
        ...
        <span style={{ fontSize: ".65rem", color: "var(--tmuted)", transform: open ? "rotate(180deg)" : "none", transition: "transform .2s" }}>▼</span>
      </button>
      {open && (
        <>
          ...
          <div style={{
            ...
            boxShadow: "0 16px 48px rgba(0,0,0,0.5)", minWidth: 160,
          }}>
            {LANGUAGES.map(lang => (
              <button
                key={lang.code}
                ...
                style={{
                  ...
                  padding: "11px 16px", background: lang.code === i18nInstance.language ? "var(--pdim)" : "transparent",
                  border: "none", color: lang.code === i18nInstance.language ? "var(--purpleL)" : "var(--t2)",
                  fontSize: ".85rem", fontWeight: lang.code === i18nInstance.language ? 700 : 400,
                  cursor: "pointer", transition: "background .15s", textAlign: "left",
                }}
                onMouseOver={e => { if (lang.code !== i18nInstance.language) e.currentTarget.style.background = "rgba(124,58,237,0.08)"; }}
                onMouseOut={e => { if (lang.code !== i18nInstance.language) e.currentTarget.style.background = "transparent"; }}
              >
                <span style={{ fontSize: "1.1rem" }}>{lang.flag}</span>
                <span>{lang.label}</span>
                {lang.code === i18nInstance.language && <span style={{ marginLeft: "auto", fontSize: ".7rem" }}>✓</span>}
              </button>
            ))}
          </div>
        </>
...
export default function Home() {
  ...
  // Apply dir on mount based on stored language
  useEffect(() => {
  ...
  const handleAiStyle = async () => {
    ...
    if (!content) { showToast(t("home.actions.generate")); return; }
    setAiLoading(true);
    setShowAiPanel(true);
    try {
  ...
  const handleGenerate = async () => {
    const content = buildQrContent(activeType, fields);
    if (!content.trim()) { setError(t("home.actions.generate")); return; }
    setError("");
    setLoading(true);
  ...
  const handleDownloadPng = () => {
    ...
    showToast(t("home.actions.download") + " ✓");
  };

  const handleDownloadSvg = () => {
    if (!qrInstance.current) return;
    qrInstance.current.download({ name: "qr-code", extension: "svg" });
    showToast(t("home.actions.downloadSvg") + " ✓");
  };
  ...
  const QR_TYPES: QrType[] = ["url", "text", "wifi", "vcard", "email", "phone", "instagram", "location", "pdf"];
    ...
    <div style={{ background: "var(--bg)", minHeight: "100vh", position: "relative" }}>
      ...
      <nav style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <a href="/pricing" style={{ fontSize: ".85rem", color: "var(--tmuted)", transition: "color .2s" }}
            ...
            onMouseOut={e => (e.currentTarget.style.color = "var(--tmuted)")}>{t("nav.pricing")}</a>
          <a href="/blog" style={{ fontSize: ".85rem", color: "var(--tmuted)", transition: "color .2s" }}
            onMouseOver={e => (e.currentTarget.style.color = "var(--white)")}
            onMouseOut={e => (e.currentTarget.style.color = "var(--tmuted)")}>{t("nav.blog")}</a>
          <LanguageSwitcher />
          {isAuthenticated ? (
      ...
      <section style={{
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center", marginBottom: 40, animation: "fadeUp .5s ease .25s both" }}>
          {[
            { dot: "var(--green)", text: t("common.free") },
            { dot: "var(--purple)", text: t("home.hero.badge") },
            { dot: "var(--amber)", text: t("home.actions.download") },
          ].map(s => (
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
            ...
            <span style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: "1.05rem", color: "var(--white)" }}>{t("home.actions.generate")}</span>
          </div>

          {/* Type Tabs */}
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 24 }}>
            {QR_TYPES.map(type => (
              <button key={type} onClick={() => { setActiveType(type); setGenerated(false); }}
                style={{
                  display: "flex", alignItems: "center", gap: 5, padding: "8px 14px",
                  borderRadius: "var(--r12)", border: "1px solid",
                  borderColor: activeType === type ? "var(--bp)" : "var(--border)",
                  background: activeType === type ? "var(--pdim)" : "transparent",
                  color: activeType === type ? "var(--purpleL)" : "var(--tmuted)",
                  fontSize: ".8rem", fontWeight: 600, transition: "all var(--tr)", cursor: "pointer",
                }}>
                <span>{icons[type]}</span>
                <span>{t(`home.qrTypes.${type}`)}</span>
              </button>
            ))}
          </div>
          ...
          <div style={{ marginBottom: 20 }}>
            {activeType === "url" && (
              <InputGroup label={t("home.fields.url")}>
            ...
            {activeType === "text" && (
              <InputGroup label={t("home.fields.text")}>
            ...
            {activeType === "wifi" && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <InputGroup label={t("home.fields.wifiSsid")}>
            ...
            {activeType === "vcard" && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <InputGroup label={t("home.fields.vcardFname")}><input type="text" placeholder="Ahmed" value={fields.vcardFname || ""} onChange={e => setField("vcardFname", e.target.value)} style={inputStyle} /></InputGroup>
                <InputGroup label={t("home.fields.vcardLname")}><input type="text" placeholder="Al-Rashid" value={fields.vcardLname || ""} onChange={e => setField("vcardLname", e.target.value)} style={inputStyle} /></InputGroup>
                <InputGroup label={t("home.fields.vcardPhone")}><input type="tel" placeholder="+966 5X XXX XXXX" value={fields.vcardPhone || ""} onChange={e => setField("vcardPhone", e.target.value)} style={inputStyle} /></InputGroup>
                <InputGroup label={t("home.fields.vcardEmail")}><input type="email" placeholder="ahmed@company.com" value={fields.vcardEmail || ""} onChange={e => setField("vcardEmail", e.target.value)} style={inputStyle} /></InputGroup>
                <InputGroup label={t("home.fields.vcardOrg")}><input type="text" placeholder="My Company" value={fields.vcardOrg || ""} onChange={e => setField("vcardOrg", e.target.value)} style={inputStyle} /></InputGroup>
                <InputGroup label={t("home.fields.vcardTitle")}><input type="text" placeholder="CEO" value={fields.vcardTitle || ""} onChange={e => setField("vcardTitle", e.target.value)} style={inputStyle} /></InputGroup>
              </div>
            )}
            {activeType === "email" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <InputGroup label={t("home.fields.emailTo")}><input type="email" placeholder="recipient@example.com" value={fields.emailTo || ""} onChange={e => setField("emailTo", e.target.value)} style={inputStyle} /></InputGroup>
                <InputGroup label={t("home.fields.emailSubject")}><input type="text" placeholder="Hello!" value={fields.emailSubject || ""} onChange={e => setField("emailSubject", e.target.value)} style={inputStyle} /></InputGroup>
                <InputGroup label={t("home.fields.emailBody")}><textarea placeholder="Your message…" value={fields.emailBody || ""} onChange={e => setField("emailBody", e.target.value)} rows={2} style={{ ...inputStyle, resize: "vertical" }} /></InputGroup>
              </div>
            )}
            {activeType === "phone" && (
              <InputGroup label={t("home.fields.phone")}>
            ...
            {activeType === "instagram" && (
              <InputGroup label={t("home.fields.igHandle")}>
                <div style={{ position: "relative" }}>
            ...
            {activeType === "location" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <InputGroup label={t("home.fields.locationName")}>
            ...
            {activeType === "pdf" && (
              <InputGroup label={t("home.fields.pdfUrl")}>
          ...
          {/* Size */}
          <div style={{ marginBottom: 20 }}>
            <label style={labelStyle}>{t("home.customize.size")}</label>
            <div style={{ display: "flex", gap: 8 }}>
              {[{ v: 160, l: "S" }, { v: 240, l: "M" }, { v: 320, l: "L" }].map(s => (
            ...
            {customOpen && (
              <div style={{ background: "var(--s2)", border: "1px solid var(--border)", borderRadius: "0 0 var(--r12) var(--r12)", padding: "16px", borderTop: "none" }}>
                <div style={{ display: "flex", gap: 16, marginBottom: 16 }}>
                  <div style={{ flex: 1 }}>
                    <label style={labelStyle}>{t("home.customize.darkColor")}</label>
                  ...
                  <div style={{ flex: 1 }}>
                    <label style={labelStyle}>{t("home.customize.lightColor")}</label>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, background: "var(--s1)", border: "1px solid var(--border)", borderRadius: "var(--r12)", padding: "8px 12px" }}>
                ...
                <div>
                  <label style={labelStyle}>{t("home.customize.logo")}</label>
                  <div
                    ...
                    {logoDataUrl ? (
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        ...
                        <button onClick={e => { e.stopPropagation(); setLogoFile(null); setLogoDataUrl(""); }}
                          style={{ marginLeft: "auto", background: "none", border: "none", color: "var(--tmuted)", cursor: "pointer", fontSize: ".75rem" }}>✕ {t("home.customize.removeLogo")}</button>
                      </div>
                    ) : (
                      <>
                        <div style={{ fontSize: "1.5rem", marginBottom: 6 }}>🖼️</div>
                        <p style={{ fontSize: ".78rem", color: "var(--tmuted)" }}>{t("home.customize.uploadLogo")}</p>
                      </>
                    )}
                  </div>
          ...
          {hasContent && (
            <div style={{ marginBottom: 20 }}>
              ...
              >
                <span>✨ {t("home.customize.aiSuggest")}</span>
                {aiLoading ? (
                  ...
                  <span style={{ fontSize: ".75rem" }}>{t("home.customize.aiTitle")} →</span>
                )}
              </button>
              {showAiPanel && !aiLoading && aiSuggestions.length > 0 && (
                <div style={{ background: "var(--s2)", border: "1px solid var(--border)", borderRadius: "0 0 var(--r12) var(--r12)", padding: "16px", borderTop: "none" }}>
                  <div style={{ fontSize: ".72rem", fontWeight: 700, letterSpacing: ".1em", textTransform: "uppercase", color: "var(--tmuted)", marginBottom: 12 }}>{t("home.customize.aiTitle")}</div>
          ...
          <button onClick={handleGenerate} disabled={loading}
            ...
            {loading ? t("home.actions.generating") : t("home.actions.generate")}
          </button>
          ...
          {generated && (
            <div style={{ marginTop: 32, animation: "fadeUp .5s ease both" }}>
              ...
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center", marginBottom: 16 }}>
                <ActionBtn onClick={handleDownloadPng} icon="⬇">{t("home.actions.download")}</ActionBtn>
                <ActionBtn onClick={handleDownloadSvg} icon="⬇">{t("home.actions.downloadSvg")}</ActionBtn>
                <ActionBtn onClick={() => setShowSaveModal(true)} icon="💾" highlight>{t("home.actions.save")}</ActionBtn>
              </div>
              ...
              <a href="/pricing" style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontFamily: "'Syne', sans-serif", fontSize: "1.05rem", fontWeight: 700, color: "var(--white)", marginBottom: 3 }}>{t("home.features.items.dynamic.title")}</div>
                    <div style={{ fontSize: ".82rem", color: "var(--tmuted)" }}>{t("home.features.items.analytics.desc")}</div>
                  </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", justifyContent: "center", marginTop: 32, paddingBottom: 64, animation: "fadeUp .7s var(--ease) .3s both" }}>
          {[
            ...
            t("home.features.items.custom.desc"),
            t("home.features.items.bulk.title"),
            t("home.features.items.api.title"),
          ].map(f => (
      ...
      <footer style={{ textAlign: "center", padding: "32px 24px", color: "var(--tdim)", fontSize: ".78rem", borderTop: "1px solid var(--border)", position: "relative", zIndex: 1 }}>
        <div style={{ marginBottom: 12, display: "flex", justifyContent: "center", gap: 24, flexWrap: "wrap" }}>
          <a href="/pricing" style={{ color: "var(--tmuted)", transition: "color .2s" }} onMouseOver={e => (e.currentTarget.style.color = "var(--white)")} onMouseOut={e => (e.currentTarget.style.color = "var(--tmuted)")}>{t("nav.pricing")}</a>
          <a href="/blog" style={{ color: "var(--tmuted)", transition: "color .2s" }} onMouseOver={e => (e.currentTarget.style.color = "var(--white)")} onMouseOut={e => (e.currentTarget.style.color = "var(--tmuted)")}>{t("nav.blog")}</a>
          <a href="/dashboard" style={{ color: "var(--tmuted)", transition: "color .2s" }} onMouseOver={e => (e.currentTarget.style.color = "var(--white)")} onMouseOut={e => (e.currentTarget.style.color = "var(--tmuted)")}>{t("nav.dashboard")}</a>
        </div>
      ...
      {showSaveModal && (
        <Modal onClose={() => setShowSaveModal(false)}>
          ...
          <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: "1.4rem", fontWeight: 800, color: "var(--white)", textAlign: "center", marginBottom: 10 }}>{t("home.save.title")}</h2>
          <p style={{ fontSize: ".9rem", color: "var(--tmuted)", textAlign: "center", lineHeight: 1.6, marginBottom: 24 }}>
            {isAuthenticated ? t("home.save.nameLabel") : t("home.save.loginRequired")}
          </p>
          {isAuthenticated ? (
            <>
              <input type="text" placeholder={t("home.save.namePlaceholder")} value={qrName}
              ...
              <button onClick={handleSave} disabled={saveQrMutation.isPending}
                ...
                {saveQrMutation.isPending ? t("home.save.saving") : t("home.save.save")}
              </button>
            </>
          ) : (
    ...
    <section style={{ position: "relative", zIndex: 2, padding: "0 24px 80px", maxWidth: 960, margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 16 }}>
        {TESTIMONIALS.map(item => (
          <div key={item.name} style={{ background: "var(--s1)", border: "1px solid var(--border)", borderRadius: 16, padding: "24px 20px", transition: "border-color var(--tr)" }}
            onMouseOver={e => (e.currentTarget.style.borderColor = "var(--bp)")}
            onMouseOut={e => (e.currentTarget.style.borderColor = "var(--border)")}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
              <div style={{ width: 42, height: 42, borderRadius: "50%", background: "linear-gradient(135deg,var(--purple),var(--purpleD))", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: ".9rem", color: "#fff", flexShrink: 0 }}>{item.avatar}</div>
              <div>
                <div style={{ fontWeight: 700, fontSize: ".9rem", color: "var(--white)" }}>{item.name}</div>
                <div style={{ fontSize: ".78rem", color: "var(--tmuted)" }}>{item.role}</div>
              </div>
            </div>
            <p style={{ fontSize: ".88rem", color: "var(--t2)", lineHeight: 1.6 }}>"{item.text}"</p>
...
function PricingSection() {
  ...
  return (
    <section style={{ position: "relative", zIndex: 2, padding: "0 24px 80px", maxWidth: 960, margin: "0 auto" }}>
      <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: "1.5rem", fontWeight: 800, color: "var(--white)", textAlign: "center", marginBottom: 8 }}>
        {t("home.pricing.title")}
      </h2>
      <p style={{ textAlign: "center", color: "var(--tmuted)", fontSize: ".9rem", marginBottom: 40 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16 }}>
        {PLANS.map(plan => (
          <div key={plan.id} style={{
            ...
            {plan.popular && (
              <div style={{ position: "absolute", top: -10, left: "50%", transform: "translateX(-50%)", background: "linear-gradient(135deg,var(--amber),#f97316)", color: "#000", fontSize: ".6rem", fontWeight: 800, letterSpacing: ".1em", textTransform: "uppercase", padding: "3px 10px", borderRadius: "var(--rfull)", whiteSpace: "nowrap" }}>
                {t("home.pricing.popular")}
              </div>
            )}
            <div style={{ fontFamily: "'Syne', sans-serif", fontSize: "1.8rem", fontWeight: 800, color: "var(--white)", marginBottom: 4 }}>{plan.price}</div>
            <div style={{ fontSize: ".75rem", color: "var(--tmuted)", marginBottom: 4 }}>
              {plan.id === "free" ? t("home.pricing.forever") : t("home.pricing.perMonth")}
            </div>
            <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, color: "var(--purpleL)", marginBottom: 16 }}>
              {t(`home.plans.${plan.id}.label`)}
            </div>
            <ul style={{ listStyle: "none", fontSize: ".72rem", color: "var(--t2)", lineHeight: 1.8, textAlign: "left" }}>
              {(t(`home.plans.${plan.id}.features`, { returnObjects: true }) as string[]).map((f: string) => (
            ...
            <a href="/pricing" style={{
              ...
              {plan.id === "free" ? t("home.pricing.getStarted") : t("home.pricing.upgrade")}
            </a>
          </div>
        ))}
      </div>
